...
EXCEPTION_HANDLING_CONFIG = {
    # 是否启用详细错误信息（仅开发环境）
    "ENABLE_DETAILED_ERRORS": config(
        "ENABLE_DETAILED_ERRORS", default=False, cast=bool
    ),
    # 是否启用异常告警
    "ENABLE_EXCEPTION_ALERTS": config(
        "ENABLE_EXCEPTION_ALERTS", default=True, cast=bool
    ),
    # 异常日志级别
...
PERFORMANCE_MONITORING_CONFIG = {
    ...
    "ENABLE_PERFORMANCE_MONITORING": config(
        "ENABLE_PERFORMANCE_MONITORING", default=True, cast=bool
    ),
    # 性能日志级别
...
RATE_LIMITING_CONFIG = {
    ...
    "SPECIAL_RATE_LIMITS": {
        ...
        "/api/v1/auth/reset-password": config(
            "RESET_PASSWORD_RATE_LIMIT", default=3, cast=int
        ),
    },
}
...
SENSITIVE_DATA_CONFIG = {
    ...
    # 是否启用敏感数据过滤
    "ENABLE_SENSITIVE_FILTERING": config(
        "ENABLE_SENSITIVE_FILTERING", default=True, cast=bool
    ),
}
...
METRICS_CONFIG = {
    ...
    # 指标存储后端
    "METRICS_BACKEND": config(
        "METRICS_BACKEND", default="memory"
