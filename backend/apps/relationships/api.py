...
class RelationshipController(StandardCRUDController):
    ...
    def serialize_object(self, obj, user=None) -> Dict[str, Any]:
        ...
        return {
            ...
            if hasattr(obj, "from_member") and obj.from_member
            else None,
            "to_member": {
                "id": obj.to_member.id,
            ...
            if hasattr(obj, "to_member") and obj.to_member
            else None,
        }
    ...
    def register_crud_routes(self) -> None:
        ...
        def list_relationships(
            request: HttpRequest, query: RelationshipQuerySchema = Query(...)
        ):
            """获取家族关系列表"""
            try:
                ...
                relationships, total = self.service_class.list_relationships(
                    user, **filters
                )
        ...
        @self.router.post(
            "/", response=ApiResponseSchema, summary="创建关系", tags=["关系管理"]
        )
        ...
        def get_relationship(request: HttpRequest, relationship_id: int = Path(...)):
            ...
            try:
                ...
                relationship = self.service_class.get_relationship(
        ...
        def delete_relationship(request: HttpRequest, relationship_id: int = Path(...)):
            ...
            try:
                ...
                return create_success_response(
                    message="关系删除成功", request_id=get_request_id(request)
                )
    ...
    def register_custom_routes(self) -> None:
        ...
        def batch_create_relationships(
            request: HttpRequest, data: RelationshipBatchCreateSchema
        ):
            """批量创建家族关系"""
            try:
                user = get_current_user(request)

                result = self.service_class.batch_create_relationships(
                ...
                return create_success_response(
                    data={
                        ...
                        "created_relationships": [
                            self.serialize_object(r, user)
                            for r in result["created_relationships"]
                        ],
        ...
        def batch_delete_relationships(
            request: HttpRequest, relationship_ids: List[int]
        ):
            """批量删除关系"""
            try:
                user = get_current_user(request)

                result = self.service_class.batch_delete_relationships(
                    relationship_ids, user
                )
        ...
        @self.router.get(
            "/member/{member_id}/relationships",
            response=ApiResponseSchema,
            summary="获取成员关系",
            tags=["关系管理"],
        )
        def get_member_relationships(request: HttpRequest, member_id: int = Path(...)):
            ...
            try:
                ...
                relationships = self.service_class.get_member_relationships(
                    member_id, user
                )
        ...
        @self.router.get(
            "/family/{family_id}/statistics",
            response=ApiResponseSchema,
            summary="获取关系统计",
            tags=["关系管理"],
        )
        def get_relationship_statistics(
            request: HttpRequest, family_id: int = Path(...)
        ):
            """获取家族关系统计信息"""
            try:
                user = get_current_user(request)
        ...
        def validate_relationship(request: HttpRequest, data: RelationshipCreateSchema):
            ...
            try:
                ...
                validation_result = self.service_class.validate_relationship(
                    data.dict(), user
                )
...
# 导出
__all__ = ["RelationshipController", "relationship_controller", "router"]
