...
class RelationshipBaseSchema(Schema):
    ...
    relationship_detail: Optional[str] = Field(
        None, max_length=100, description="关系详情"
    )
...
class RelationshipSearchSchema(PaginationQuerySchema):
    ...
    is_active: Optional[bool] = Field(None, description="是否激活")
    confidence_level_min: Optional[int] = Field(
        None, ge=1, le=10, description="最低可信度"
    )
    confidence_level_max: Optional[int] = Field(
        None, ge=1, le=10, description="最高可信度"
    )
