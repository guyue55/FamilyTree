...
class RelationshipService(BaseService, CacheableService):
    ...
    def validate_create_data(self, data: Dict[str, Any], user=None) -> Dict[str, Any]:
        ...
        # 检查权限
        if not from_member.family.can_user_manage(
            user
        ) or not to_member.family.can_user_manage(user):
    ...
    def validate_update_data(
        self, obj, data: Dict[str, Any], user=None
    ) -> Dict[str, Any]:
        """验证关系更新数据"""
        # 检查权限
    ...
    def check_permissions(self, obj, user, action: str) -> bool:
        """检查关系权限"""
        if action == "view":
            return obj.from_member.can_user_view(user) and obj.to_member.can_user_view(
                user
            )
    ...
    def create_relationship(self, data: Dict[str, Any], user: User) -> Relationship:
        ...
        try:
            ...
            logger.info(
                f"Relationship created: {relationship.from_member.name} -> {relationship.to_member.name} ({relationship.relationship_type}) by {user.username}"
            )
            return relationship
    ...
    def list_relationships(
        ...
        try:
            ...
            relationships = list(
                queryset.select_related("from_member", "to_member", "created_by")[
                    start:end
                ]
            )
    ...
    def get_member_relationships(self, member_id: int, user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取所有相关关系
            outgoing = Relationship.objects.filter(from_member=member).select_related(
                "to_member"
            )
            incoming = Relationship.objects.filter(to_member=member).select_related(
                "from_member"
            )
            ...
            for rel in outgoing:
                rel_data = {
                    ...
                    "relationship_name": self.RELATIONSHIP_TYPES.get(
        ...
        try:
            ...
            for relationship in relationships:
                edge = {
                    ...
                    "label": self.RELATIONSHIP_TYPES.get(
        ...
        try:
            ...
            for rel_type, rel_name in self.RELATIONSHIP_TYPES.items():
                count = Relationship.objects.filter(
                    from_member__family_id=family_id, relationship_type=rel_type
                ).count()
                relationship_stats[rel_type] = {"name": rel_name, "count": count}

            # 统计世代信息
            generation_stats = (
                Member.objects.filter(family_id=family_id)
                .values("generation")
    ...
    def validate_relationship(
        self, from_member_id: int, to_member_id: int, relationship_type: str, user: User
    ) -> Dict[str, Any]:
        """验证关系的合理性"""
        try:
            from_member = Member.objects.get(id=from_member_id)
            ...
            if (
                ...
                and from_member.generation <= to_member.generation
            ):
    ...
    def batch_create_relationships(
        self, relationships_data: List[Dict[str, Any]], user: User
    ) -> Dict[str, Any]:
        """批量创建关系"""
    ...
    def batch_delete_relationships(
        self, relationship_ids: List[int], user: User
    ) -> Dict[str, Any]:
        """批量删除关系"""
        deleted_count = 0
        errors = []
        ...
        for relationship_id in relationship_ids:
            try:
                ...
                errors.append(
                    {"relationship_id": relationship_id, "error": "关系不存在"}
                )
            except Exception as e:
                errors.append({"relationship_id": relationship_id, "error": str(e)})
