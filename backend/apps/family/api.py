...
class FamilyController(StandardCRUDController):
    ...
    def register_crud_routes(self) -> None:
        ...
        @self.router.post(
            "/", response=ApiResponseSchema, summary="创建家族", tags=["家族管理"]
        )
    ...
    def register_custom_routes(self) -> None:
        ...
        def search_public_families(
            request: HttpRequest, query: PublicFamilyQuerySchema = Query(...)
        ):
            """搜索公开家族"""
            try:
                # 构建搜索过滤器，利用Schema的数据
                ...
                # 使用Schema序列化数据
                data = [
                    FamilyResponseSchema.from_orm(family).dict() for family in families
                ]
        ...
        def update_family_settings(
            request: HttpRequest,
            settings_data: FamilySettingsSchema,
            family_id: int = Path(...),
        ):
            """更新家族设置"""
            try:
                ...
                settings = self.service_class.update_family_settings(
                    family_id, update_data, user
                )
        ...
        def create_family_invitation(
            request: HttpRequest,
            invitation_data: FamilyInvitationCreateSchema,
            family_id: int = Path(...),
        ):
            """创建家族邀请"""
            try:
                ...
                invitation = self.service_class.create_family_invitation(
                    family_id=family_id, invitation_data=invitation_dict, user=user
                )
        ...
        def get_invitation(request: HttpRequest, invitation_id: int = Path(...)):
            ...
            try:
                ...
                # 使用Service层获取邀请详情
                invitation = self.service_class.get_invitation_detail(
                    invitation_id, user
                )
