...
class FamilyBaseSchema(Schema, BaseConfig):
    ...
    visibility: FamilyVisibility = Field(
        FamilyVisibility.FAMILY, description="可见性设置"
    )
...
class FamilyUpdateSchema(Schema, BaseConfig):
    ...
    name: Optional[str] = Field(
        None, min_length=2, max_length=100, description="家族名称"
    )
    description: Optional[str] = Field(None, max_length=1000, description="家族描述")
...
class FamilySettingsSchema(Schema, BaseConfig):
    ...
    theme_color: str = Field(
        "#1890ff", pattern=r"^#[0-9A-Fa-f]{6}$", description="主题颜色"
    )
...
class FamilyInvitationCreateSchema(Schema, BaseConfig):
    ...
    invitee_phone: Optional[str] = Field(
        None, pattern=r"^1[3-9]\d{9}$", description="被邀请者手机号"
    )
...
class FamilyInvitationProcessSchema(Schema, BaseConfig):
    ...
    rejection_reason: Optional[str] = Field(
        None, max_length=200, description="拒绝原因"
    )
...
class FamilyStatisticsSchema(Schema, BaseConfig):
    ...
    top_families_by_members: List[Dict[str, Any]] = Field(
        ..., description="成员数最多的家族"
    )
...
class FamilyJoinRequestSchema(Schema, BaseConfig):
    ...
    relationship_to_family: Optional[str] = Field(
        None, max_length=100, description="与家族的关系"
    )
...
class FamilyQuerySchema(PaginationQuerySchema):
    ...
    visibility: Optional[str] = Field(
        None, pattern=r"^(public|private|members_only)$", description="可见性过滤"
    )
...
__all__ = [
    ...
    "FamilyJoinRequestResponseSchema",
    "FamilyActivityLogSchema",
    # 路径参数Schema
    "FamilyPathSchema",
