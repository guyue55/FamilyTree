"""
家族模块 Django Ninja Schema 定义

该模块定义了家族相关的输入输出数据结构，用于API接口的数据验证和序列化。
遵循Django Ninja最佳实践和Google Python Style Guide。
利用Django Ninja的高级功能优化Schema定义，减少冗余代码。
"""
...
class FamilyModelSchema(ModelSchema):
    ...
    class Config:
        model = Family
        model_fields = [
            'id', 'name', 'description', 'creator_id', 'avatar', 'cover_image',
            'visibility', 'is_active', 'allow_join', 'member_count', 
            'generation_count', 'tags', 'origin_location', 'motto',
