...
class FamilyTestDataGenerator:
    ...
    @staticmethod
    def create_family_with_settings(creator: User = None, **kwargs) -> tuple[Family, FamilySettings]:
        """创建带设置的家族"""
    ...
    def _create_basic_family_scenario() -> Dict[str, Any]:
        """创建基本家族场景"""
        creator = UserFactory()
        family, settings = FamilyTestDataGenerator.create_family_with_settings(creator=creator)
    ...
    def _create_multi_family_scenario() -> Dict[str, Any]:
        ...
        for i, user in enumerate(users):
            family = FamilyTestDataGenerator.create_family(
                creator=user,
                visibility=['public', 'family', 'private'][i]
            )
...
class FamilyTestUtils:
    ...
    def assert_family_data(family: Family, expected_data: Dict[str, Any]):
        ...
        for field, expected_value in expected_data.items():
            ...
            assert actual_value == expected_value, f"字段 {field} 不匹配: 期望 {expected_value}, 实际 {actual_value}"
    ...
    def assert_family_settings(settings: FamilySettings, expected_data: Dict[str, Any]):
        ...
        for field, expected_value in expected_data.items():
            ...
            assert actual_value == expected_value, f"设置字段 {field} 不匹配: 期望 {expected_value}, 实际 {actual_value}"
    
    @staticmethod
    def assert_invitation_data(invitation: FamilyInvitation, expected_data: Dict[str, Any]):
        """断言邀请数据"""
        for field, expected_value in expected_data.items():
            actual_value = getattr(invitation, field)
            assert actual_value == expected_value, f"邀请字段 {field} 不匹配: 期望 {expected_value}, 实际 {actual_value}"
...
if __name__ == '__main__':
    ...
    print(f"支持的测试场景: {list(FamilyTestDataGenerator.create_test_scenario.__annotations__.keys())}")
