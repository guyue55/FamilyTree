...
class TestFamilyService(TestCase):
    ...
    def test_update_family_success(self):
        ...
        updated_family = FamilyService.update_family(
            family.id, update_data, self.user1
        )
    ...
    def test_list_families_with_filters(self):
        ...
        # 测试可见性过滤
        families, total = FamilyService.list_families(
            self.user1,
            visibility='public'
        )
    ...
    def test_list_families_search(self):
        ...
        self.create_test_family(
            name='北京张氏家族',
            description='来自北京的张氏家族',
            tags='北京,张氏'
        )
        self.create_test_family(
            name='上海李氏家族',
            description='来自上海的李氏家族',
            tags='上海,李氏'
        )
...
class TestFamilyServicePerformance:
    ...
    def test_family_search_performance(self):
        ...
        # 验证搜索结果的准确性
        for family in families:
            assert ('北京' in family.name or 
                   '北京' in family.description or 
                   '北京' in (family.tags or ''))
