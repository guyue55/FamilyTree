...
class TestFamilyAPIPerformance(APITestCase):
    ...
    def test_family_update_performance(self):
        ...
        update_data = {"description": "更新后的描述", "motto": "新的座右铭"}
...
class TestFamilyDatabasePerformance:
    ...
    def setup_method(self, method):
        ...
        self.user = User.objects.create_user(
            username="dbperfuser", email="dbperf@example.com", password="testpass123"
        )

        # 创建测试数据
        self.families = []
        for i in range(100):
            family = Family.objects.create(name=f"数据库性能测试{i}", creator=self.user)
            self.families.append(family)
    ...
    def test_bulk_operations_performance(self):
        ...
        bulk_families = []
        for i in range(100):
            bulk_families.append(Family(name=f"批量创建{i}", creator=self.user))
        ...
        for i in range(10):  # 只创建10个进行对比
            Family.objects.create(name=f"逐个创建{i}", creator=self.user)
...
class TestFamilyCachePerformance:
    ...
    def setup_method(self):
        ...
        self.family = Family.objects.create(name="缓存性能测试家族", creator=self.user)
...
class TestFamilyConcurrencyPerformance:
    ...
    def test_concurrent_family_creation(self):
        ...
        def create_family(index):
            try:
                family = Family.objects.create(
                    name=f"并发创建家族{index}", creator=self.user
                )
                return family.id
...
class TestFamilyMemoryPerformance:
    ...
    def setup_method(self):
        """测试方法初始化"""
        self.user = User.objects.create_user(
            username="memoryuser", email="memory@example.com", password="testpass123"
        )
    ...
    def test_queryset_memory_efficiency(self):
        ...
        # 创建测试数据
        for i in range(500):
            Family.objects.create(name=f"查询集测试{i}", creator=self.user)
    ...
    def test_object_lifecycle_memory(self):
        ...
        # 创建对象并获取弱引用
        family = Family.objects.create(name="生命周期测试", creator=self.user)
