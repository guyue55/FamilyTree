...
from ..schemas import (
    FamilyCreateSchema, FamilyUpdateSchema, FamilySettingsSchema,
    FamilyInvitationCreateSchema, FamilyFilterSchema
)
...
class TestFamilyAPIIntegration(APITestCase):
    ...
    def test_complete_family_lifecycle(self):
        ...
        # 7. 获取家族统计
        response = self.api_get(
            f'/families/{family_id}/statistics/'
        )
        self.assert_api_success(response)
    ...
    def test_family_member_management_flow(self):
        ...
        # 4. 验证成员已加入
        response = self.api_get(
            f'/api/families/{self.family1.id}/members/',
            user=self.user1
        )
        self.assert_success(response)
...
class TestFamilyExternalServiceIntegration:
    ...
    def test_file_storage_integration(self, mock_storage_service):
        ...
        mock_storage_instance.upload_avatar.return_value = 'https://example.com/avatar.jpg'
...
class TestFamilyEndToEndScenarios:
    ...
    def test_family_creation_and_growth_scenario(self):
        ...
        assert FamilyMembership.objects.filter(family=family, status='active').count() == 2
        assert FamilyInvitation.objects.filter(family=family, status='accepted').count() == 2
