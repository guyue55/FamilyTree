...
class TestFamilyPermissions:
    ...
    def setup_method(self, method):
        ...
        FamilyMembership.objects.create(
            family=self.family_visible, user=self.admin, role="admin", status="active"
        )

        FamilyMembership.objects.create(
            family=self.family_visible, user=self.member, role="member", status="active"
        )
    ...
    def test_member_permissions(self):
        """测试普通成员权限"""
        # 普通成员只有查看和导出权限
        member_permissions = [FamilyPermission.VIEW, FamilyPermission.EXPORT_DATA]
...
class TestFamilySecurityMechanisms:
    ...
    def setup_method(self):
        ...
        self.family = Family.objects.create(
            name="安全测试家族", creator=self.user1, visibility="private"
        )
    ...
    def test_sql_injection_prevention(self):
        ...
        # 创建家族时使用恶意输入
        try:
            Family.objects.create(name=malicious_input, creator=self.user1)
        except Exception:
            pass  # 预期可能失败
    ...
    def test_xss_prevention_in_data(self):
        ...
        family = Family.objects.create(
            name="XSS测试家族", description=xss_input, creator=self.user1
        )
        ...
        for i in range(10):  # 尝试创建10个家族
            try:
                if families_created < max_families:
                    Family.objects.create(name=f"速率测试家族{i}", creator=self.user1)
                    families_created += 1
                else:
                    # 模拟速率限制
    ...
    def test_password_protection_in_logs(self):
        ...
        # 这里只是示例，实际应该检查日志系统
        filtered_data = {
            k: v if k != "password" else "***" for k, v in sensitive_data.items()
        }
...
class TestFamilyPermissionEdgeCases:
    ...
    def setup_method(self):
        ...
        self.family = Family.objects.create(name="边界测试家族", creator=self.user)
    ...
    def test_concurrent_permission_changes(self):
        ...
        membership = FamilyMembership.objects.create(
            family=self.family, user=self.user, role="member", status="active"
        )
...
class TestFamilyErrorHandling:
    ...
    def test_error_context_preservation(self):
        ...
        error = FamilyValidationError("验证失败", details=context)
