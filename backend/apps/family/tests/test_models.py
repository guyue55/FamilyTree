...
class TestFamilyModel:
    ...
    def test_family_avatar_upload(self):
        ...
        avatar_file = SimpleUploadedFile(
            "avatar.jpg", b"fake_image_content", content_type="image/jpeg"
        )

        family = Family.objects.create(**self.family_data, avatar=avatar_file)
    ...
    def test_family_cover_image_upload(self):
        ...
        cover_file = SimpleUploadedFile(
            "cover.jpg", b"fake_cover_content", content_type="image/jpeg"
        )

        family = Family.objects.create(**self.family_data, cover_image=cover_file)
    ...
    def test_family_settings_property(self):
        ...
        settings = FamilySettings.objects.create(
            family=family, tree_layout="horizontal", show_photos=False
        )
...
class TestFamilySettingsModel:
    ...
    def setup_method(self, method):
        ...
        self.family = Family.objects.create(name="设置测试家族", creator=self.user)
...
class TestFamilyInvitationModel:
    ...
    def setup_method(self, method):
        ...
        self.inviter = User.objects.create_user(
            username="inviter", email="inviter@example.com", password="testpass123"
        )
        ...
        self.family = Family.objects.create(name="邀请测试家族", creator=self.inviter)
...
class TestFamilyModelConstraints:
    ...
    def test_family_name_length_constraint(self):
        ...
        family = Family.objects.create(name=max_length_name, creator=self.user)
        assert len(family.name) == 100

        # 测试超过最大长度
        with pytest.raises(Exception):
            Family.objects.create(name="A" * 101, creator=self.user)
    ...
    def test_family_description_length_constraint(self):
        ...
        family = Family.objects.create(
            name="描述长度测试", description=max_length_desc, creator=self.user
        )
        assert len(family.description) == 1000

    def test_family_member_count_constraint(self):
        """测试家族成员数量约束"""
        family = Family.objects.create(name="成员数量约束测试", creator=self.user)
    ...
    def test_family_generation_count_constraint(self):
        """测试家族世代数量约束"""
        family = Family.objects.create(name="世代数量约束测试", creator=self.user)
...
class TestFamilyModelMethods:
    ...
    def setup_method(self):
        """测试方法初始化"""
        self.user = User.objects.create_user(
            username="methoduser", email="method@example.com", password="testpass123"
        )

        self.family = Family.objects.create(
            name="方法测试家族", creator=self.user, tags="测试,方法,家族"
        )
    ...
    def test_update_member_count_method(self):
        ...
        FamilyMembership.objects.create(
            family=self.family, user=self.user, role="admin", status="active"
        )
        ...
        FamilyMembership.objects.create(
            family=self.family, user=user2, role="member", status="active"
        )
