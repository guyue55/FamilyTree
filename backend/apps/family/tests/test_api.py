...
class FamilyAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        FamilyMember.objects.create(
            family=self.test_family, user=self.test_user, role="admin", status="active"
        )
...
class TestFamilyCRUDAPI(FamilyAPITestCase):
    ...
    def test_create_family_success(self):
        """测试创建家族成功"""
        family_data = {"name": "新测试家族", "description": "这是一个新的测试家族"}
    ...
    def test_update_family_success(self):
        """测试更新家族成功"""
        update_data = {"name": "更新后的家族名称", "description": "更新后的描述"}

        response = self.api_put(
            f"/family/{self.test_family.id}/", data=update_data, authenticated=True
        )
    ...
    def test_delete_family_success(self):
        ...
        response = self.api_delete(
            f"/family/{self.test_family.id}/", authenticated=True
        )
...
class TestFamilyPublicSearchAPI(FamilyAPITestCase):
    ...
    def test_search_public_families_success(self):
        ...
        response = self.api_get(
            "/family/public/", params={"search": "测试"}, authenticated=True
        )
...
class TestFamilyMembershipAPI(FamilyAPITestCase):
    ...
    def test_join_family_success(self):
        ...
        response = self.api_post(
            f"/family/{public_family.id}/join/", authenticated=True
        )
    ...
    def test_leave_family_success(self):
        """测试离开家族成功"""
        response = self.api_post(
            f"/family/{self.test_family.id}/leave/", authenticated=True
        )
...
class TestFamilySettingsAPI(FamilyAPITestCase):
    ...
    def test_get_family_settings_success(self):
        """测试获取家族设置成功"""
        response = self.api_get(
            f"/family/{self.test_family.id}/settings/", authenticated=True
        )
        ...
        if response.status_code == 200:
            ...
            # 如果返回其他状态码，记录实际响应
            print(
                f"Get family settings response: {response.status_code}, {response.content}"
            )

    def test_update_family_settings_success(self):
        """测试更新家族设置成功"""
        settings_data = {"allow_join": False, "visibility": "private"}

        response = self.api_put(
            f"/family/{self.test_family.id}/settings/",
            data=settings_data,
            authenticated=True,
        )
        ...
        if response.status_code == 200:
            ...
            # 如果返回其他状态码，记录实际响应
            print(
                f"Update family settings response: {response.status_code}, {response.content}"
            )
...
class TestFamilyInvitationAPI(FamilyAPITestCase):
    ...
    def test_list_family_invitations_success(self):
        """测试获取家族邀请列表成功"""
        response = self.api_get(
            f"/family/{self.test_family.id}/invitations/", authenticated=True
        )
        ...
        if response.status_code == 200:
            ...
            # 如果返回其他状态码，记录实际响应
            print(
                f"List invitations response: {response.status_code}, {response.content}"
            )
    ...
    def test_create_family_invitation_success(self):
        ...
        response = self.api_post(
            f"/family/{self.test_family.id}/invitations/",
        ...
        if response.status_code in [200, 201]:
            ...
            # 如果返回其他状态码，记录实际响应
            print(
                f"Create invitation response: {response.status_code}, {response.content}"
            )
...
class TestFamilyStatisticsAPI(FamilyAPITestCase):
    ...
    def test_get_family_statistics_success(self):
        """测试获取家族统计成功"""
        response = self.api_get(
            f"/family/{self.test_family.id}/statistics/", authenticated=True
        )
        ...
        if response.status_code == 200:
            ...
            # 如果返回其他状态码，记录实际响应
            print(
                f"Get statistics response: {response.status_code}, {response.content}"
            )
...
class TestFamilyPermissionAPI(FamilyAPITestCase):
    ...
    def test_get_family_permissions_success(self):
        """测试获取家族权限成功"""
        response = self.api_get(
            f"/family/{self.test_family.id}/permissions/", authenticated=True
        )
        ...
        if response.status_code == 200:
            ...
            # 如果返回其他状态码，记录实际响应
            print(
                f"Get permissions response: {response.status_code}, {response.content}"
            )
...
class TestFamilyPaginationAPI(FamilyAPITestCase):
    ...
    def test_pagination_parameters(self):
        """测试分页参数"""
        response = self.api_get(
            "/family/", params={"page": 1, "page_size": 10}, authenticated=True
        )
