...
class Family(SoftDeleteModel):
    ...
    def settings(self):
        ...
        try:
            ...
            return FamilySettings.objects.create(
                family_id=self.id, tree_layout="vertical", default_generations=5
            )
    ...
    def update_member_count(self):
        ...
        count = FamilyMembership.objects.filter(
            family_id=self.id, status="active"
        ).count()
...
class FamilyInvitation(BaseModel):
    ...
    invitee_email = models.EmailField(
        verbose_name="被邀请者邮箱", db_comment="被邀请者的邮箱地址"
    )
    ...
    expires_at = models.DateTimeField(
        verbose_name="过期时间", help_text="邀请的过期时间", db_comment="邀请的过期时间"
    )
