...
class EnhancedExceptionHandler:
    ...
    def handle_exception(self, request: HttpRequest, exception: Exception) -> Dict[str, Any]:
        """
        统一异常处理入口
    ...
    def _record_exception_stats(self, exception: Exception, request: HttpRequest) -> None:
        """记录异常统计信息"""
        try:
    ...
    def _collect_request_data(self, request: HttpRequest) -> Dict[str, Any]:
        ...
        if (request.method in ['POST', 'PUT', 'PATCH'] and
            EXCEPTION_HANDLING_CONFIG['LOG_REQUEST_BODY']):
            try:
        ...
        # 过滤敏感头部信息
        request_data['headers'] = self.sensitive_filter.filter_headers(request_data['headers'])
    ...
    def _get_log_level(self, exception: Exception) -> str:
        ...
        if isinstance(exception, BaseApplicationException):
            ...
            return 'critical'
        elif isinstance(exception, (DjangoValidationError, NinjaValidationError, PydanticValidationError)):
            return 'info'
        else:
            return 'error'

    def _handle_critical_exception(self, request: HttpRequest, exception: Exception, request_id: str) -> None:
        """处理关键异常"""
        try:
            # 记录关键异常
            logger.critical(
                ...
                extra={
                    ...
                    'method': request.method,
                    'user_id': getattr(request.user, 'id', None) if hasattr(request, 'user') else None,
                    'traceback': traceback.format_exc() if EXCEPTION_HANDLING_CONFIG['ENABLE_DETAILED_ERRORS'] else None
                }
            )
    ...
    def _trigger_alert(self, exception: Exception, request: HttpRequest, request_id: str) -> None:
        """触发异常告警"""
        try:
            alert_message = (
    ...
    def _create_error_response(self, exception: Exception, request_id: str, timestamp: datetime) -> ErrorResponseSchema:
        ...
        if isinstance(exception, IntegrityError):
            return ErrorResponseSchema(
                ...
                data={'api_code': ApiErrorCode.VALIDATION_ERROR, 'detail': '数据完整性约束违反', 'type': 'integrity_error'},
                timestamp=timestamp,
                request_id=request_id
            )
        ...
        if isinstance(exception, OperationalError):
            return ErrorResponseSchema(
                ...
                data={'api_code': ApiErrorCode.SERVICE_UNAVAILABLE, 'detail': '数据库操作错误', 'type': 'operational_error'},
                timestamp=timestamp,
                request_id=request_id
            )
        ...
        if isinstance(exception, DatabaseError):
            return ErrorResponseSchema(
                code=503,
                message=ApiErrorMessage.get_message(ApiErrorCode.SERVICE_UNAVAILABLE),
                data={'api_code': ApiErrorCode.SERVICE_UNAVAILABLE, 'detail': '数据库服务异常', 'type': 'database_error'},
                timestamp=timestamp,
                request_id=request_id
            )
        ...
        if isinstance(exception, MultipleObjectsReturned):
            return ErrorResponseSchema(
                code=409,
                message=ApiErrorMessage.get_message(ApiErrorCode.VALIDATION_ERROR),
                data={'api_code': ApiErrorCode.VALIDATION_ERROR, 'detail': '查询返回多个对象，期望唯一结果'},
                timestamp=timestamp,
                request_id=request_id
            )
        ...
        if isinstance(exception, RequestDataTooBig):
            return ErrorResponseSchema(
                code=413,
                message=ApiErrorMessage.get_message(ApiErrorCode.VALIDATION_ERROR),
                data={'api_code': ApiErrorCode.VALIDATION_ERROR, 'detail': '请求数据过大'},
                timestamp=timestamp,
                request_id=request_id
            )
        ...
        if isinstance(exception, TooManyFieldsSent):
            return ErrorResponseSchema(
                code=400,
                message=ApiErrorMessage.get_message(ApiErrorCode.VALIDATION_ERROR),
                data={'api_code': ApiErrorCode.VALIDATION_ERROR, 'detail': '请求字段过多'},
                timestamp=timestamp,
                request_id=request_id
            )
        ...
        if isinstance(exception, DjangoValidationError):
            return self._handle_django_validation_error(exception, request_id, timestamp)
        ...
        if isinstance(exception, PydanticValidationError):
            return self._handle_pydantic_validation_error(exception, request_id, timestamp)

        # 处理其他未知异常
        return self._handle_unknown_exception(exception, request_id, timestamp)

    def _handle_django_validation_error(self, exception: DjangoValidationError, request_id: str, timestamp: datetime) -> ErrorResponseSchema:
        """处理Django验证异常"""
        errors = []
    ...
    def _handle_ninja_validation_error(self, exception: NinjaValidationError, request_id: str, timestamp: datetime) -> ErrorResponseSchema:
        """处理Ninja验证异常"""
        errors = []
        ...
        return ErrorResponseSchema(
            code=400,
            message=ApiErrorMessage.get_message(ApiErrorCode.VALIDATION_ERROR),
            data={'api_code': ApiErrorCode.VALIDATION_ERROR, 'detail': 'API参数验证失败'},
            errors=errors,
            timestamp=timestamp,
            request_id=request_id
        )

    def _handle_pydantic_validation_error(self, exception: PydanticValidationError, request_id: str, timestamp: datetime) -> ErrorResponseSchema:
        """处理Pydantic验证异常"""
        errors = []
        for error in exception.errors():
            error_info = {
        ...
        return ErrorResponseSchema(
            code=400,
            message=ApiErrorMessage.get_message(ApiErrorCode.VALIDATION_ERROR),
            data={'api_code': ApiErrorCode.VALIDATION_ERROR, 'detail': '数据模型验证失败'},
            errors=errors,
            timestamp=timestamp,
            request_id=request_id
        )

    def _handle_unknown_exception(self, exception: Exception, request_id: str, timestamp: datetime) -> ErrorResponseSchema:
        """处理未知异常"""
        if settings.DEBUG:
            # 开发环境显示详细错误信息
            return ErrorResponseSchema(
            ...
            return ErrorResponseSchema(
                ...
                data={'api_code': ApiErrorCode.INTERNAL_SERVER_ERROR, 'detail': '服务器内部错误'},
                timestamp=timestamp,
                request_id=request_id
            )
    ...
    def _get_response_headers(self, exception: Exception) -> Dict[str, str]:
        ...
        # 为特定异常添加特殊头部
        if isinstance(exception, RequestDataTooBig):
            headers['X-Max-Request-Size'] = str(getattr(settings, 'DATA_UPLOAD_MAX_MEMORY_SIZE', 2621440))
