...
该文件提供测试相关的工具函数和类。
遵循Django测试最佳实践和Google Python Style Guide。
"""
...
class APITestCase(TestCase):
    """API测试基类 - 使用Django Client确保中间件执行"""
    ...
    def create_test_user(self, **kwargs) -> User:
        ...
        # 生成符合格式的手机号 (1开头，第二位3-9，总共11位)
        phone_number = f"1{random.randint(3, 9)}{random.randint(10000000, 99999999)}"
        ...
        # 确保phone字段符合格式要求（11位，以1开头，第二位3-9）
        if 'phone' in default_data:
            phone = default_data['phone']
            if not phone.startswith('1') or len(phone) != 11 or not phone[1:].isdigit() or phone[1] not in '3456789':
                # 重新生成符合要求的手机号
                default_data['phone'] = f"1{random.randint(3, 9)}{random.randint(10000000, 99999999)}"
    ...
    def authenticate_user(self, user: Optional[User] = None) -> str:
        ...
        # 使用Django Client需要完整路径和正确的content_type
        ...
        if 'data' in response_data and 'access_token' in response_data['data']:
            ...
            raise ValueError(f"Cannot find access_token in response: {response_data}")
    ...
    def api_request(
        ...
        **kwargs
        ...
        # 处理认证头
        if authenticated:
            kwargs.update(self.get_auth_headers())
