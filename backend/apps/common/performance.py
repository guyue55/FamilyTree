...
class SimplePerformanceTracker:
    ...
    @staticmethod
    def record_request(
        endpoint: str, method: str, response_time: float, status_code: int
    ) -> None:
        """记录请求性能数据"""
        cache_key = f"perf_{method}_{endpoint}"

        # 获取现有统计
        stats = cache.get(cache_key, {"count": 0, "total_time": 0.0, "error_count": 0})
...
def get_endpoint_stats(endpoint: Optional[str] = None) -> Dict[str, Any]:
    ...
    if endpoint:
        ...
        stats = cache.get(cache_key, {})
        if stats:
            return {"endpoint": endpoint, "stats": stats}

    return {"endpoint": endpoint, "message": "No statistics available"}
