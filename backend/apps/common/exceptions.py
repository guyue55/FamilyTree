...
def raise_if_permission_denied(
    has_permission: bool, required_permission: str, user_role: str = None
):
    """
    如果权限不足则抛出异常
    ...
    if not has_permission:
        raise PermissionError(
            f"需要 {required_permission} 权限", required_permission, user_role
        )


def raise_if_limit_exceeded(current_value: int, max_value: int, limit_type: str):
    """
    如果超出限制则抛出异常
