...
class MediaService(BaseService, CacheableService):
    ...
    def _filter_by_permission(cls, queryset: QuerySet, user: User) -> QuerySet:
        ...
        return queryset.filter(
            Q(visibility='public') |
            Q(uploader=user) |
            Q(family__members=user)
        ).distinct()
    
    def upload_media(self, file: UploadedFile, data: Dict[str, Any], user: User) -> MediaFile:
        """
        上传媒体文件
    ...
    def batch_upload_media(self, files: List[UploadedFile], data: Dict[str, Any], user: User) -> Dict[str, Any]:
        """
        批量上传媒体文件
    ...
    def update_media(self, media_id: int, data: Dict[str, Any], user: User) -> MediaFile:
        """
        更新媒体文件信息
    ...
    def batch_update_tags(self, media_ids: List[int], tags: List[str], user: User) -> Dict[str, Any]:
        """
        批量更新标签
    ...
    def get_media_statistics(self, user: User, family_id: Optional[int] = None) -> Dict[str, Any]:
        """
        获取媒体统计信息
        ...
        for file_type in ['image', 'video', 'audio', 'document']:
            file_type_stats[f'{file_type}_count'] = queryset.filter(file_type=file_type).count()
    ...
    @classmethod
    def validate_create_data(cls, data: Dict[str, Any], user: User = None) -> Dict[str, Any]:
        """
        验证创建数据
        ...
        if data.get('visibility') and data['visibility'] not in valid_visibility:
            raise ValidationError(f"无效的可见性设置，必须是: {', '.join(valid_visibility)}")
    ...
    @classmethod
    def validate_update_data(cls, data: Dict[str, Any], user: User = None) -> Dict[str, Any]:
        """
        验证更新数据
