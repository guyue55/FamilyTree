...
class MediaFile(SoftDeleteModel):
    ...
    view_count = models.PositiveIntegerField(
        default=0, verbose_name="查看次数", db_comment="媒体文件的查看次数"
    )

    download_count = models.PositiveIntegerField(
        default=0, verbose_name="下载次数", db_comment="媒体文件的下载次数"
    )
...
class MediaMemberTag(BaseModel):
    ...
    confirmed_at = models.DateTimeField(
        blank=True, null=True, verbose_name="确认时间", db_comment="标记确认的时间"
    )
...
class MediaAlbum(SoftDeleteModel):
    ...
    description = models.TextField(
        max_length=500, blank=True, verbose_name="相册描述", db_comment="相册描述"
    )
    ...
    view_count = models.PositiveIntegerField(
        default=0, verbose_name="查看次数", db_comment="查看次数"
    )
...
class MediaComment(BaseModel):
    ...
    content = models.TextField(
        max_length=1000, verbose_name="评论内容", db_comment="评论内容"
    )
    ...
    like_count = models.PositiveIntegerField(
        default=0, verbose_name="点赞数", db_comment="点赞数"
    )
