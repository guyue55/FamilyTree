...
class MemberService(BaseService, CacheableService):
    ...
    def batch_create_members(
        self, members_data: List[Dict[str, Any]], user: User
    ) -> Dict[str, Any]:
        """批量创建成员"""
        created_members = []
        errors = []
        ...
        for member_id in member_ids:
            try:
                ...
                if self.check_permissions(member, user, "delete"):
                    ...
                    errors.append({"member_id": member_id, "error": "无权限删除此成员"})
            except Member.DoesNotExist:
                errors.append({"member_id": member_id, "error": "成员不存在"})
            except Exception as e:
    ...
    def get_family_tree(self, family_id: int, user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取所有成员
            members = Member.objects.filter(family_id=family_id).order_by(
            ...
            for member in members:
                member_data = {
                    ...
                    "death_date": member.death_date.isoformat()
