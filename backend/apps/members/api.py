...
class MemberController(StandardCRUDController):
    ...
    def register_crud_routes(self) -> None:
        ...
        @self.router.post(
            "/", response=ApiResponseSchema, summary="创建成员", tags=["成员管理"]
        )
        ...
        def delete_member(request: HttpRequest, member_id: int = Path(...)):
            ...
            try:
                ...
                return create_success_response(
                    message="成员删除成功", request_id=get_request_id(request)
                )
                ...
                return create_success_response(
                    data={
                        ...
                        "created_members": [
                            ...
                            for m in result["created_members"]
                        ],
            ...
            try:
                ...
                for rel in rel_qs:
                    if rel.relationship_type == "parent":
                        children_map.setdefault(rel.from_member_id, []).append(
                            rel.to_member_id
                        )
                ...
                def pick_parent(member_id: int):
                    ...
                    males = [
                        pid
                        for pid in ids
                        if any(mm.id == pid and mm.gender == "male" for mm in members)
                    ]
                    ...
                    data.append(
                        {
                            ...
                            "gender": "male"
                            if m.gender == "male"
                            else ("female" if m.gender == "female" else "male"),
                            "birthDate": m.birth_date.isoformat()
                            if m.birth_date
                            else None,
                            "deathDate": m.death_date.isoformat()
                            if m.death_date
                            else None,
                            "generation": m.generation,
                            "parentId": str(pick_parent(m.id))
                            if pick_parent(m.id) is not None
                            else None,
                            "spouseId": str(pick_spouse(m.id))
                            if pick_spouse(m.id) is not None
                            else None,
                            "children": [
                                str(cid) for cid in (children_map.get(m.id) or [])
                            ],
                        }
                    )
...
__all__ = ["MemberController", "member_controller", "router"]
