...
from apps.common.schemas import ApiResponseSchema, PaginatedApiResponseSchema
...
class UserController(StandardCRUDController):
    ...
    def serialize_object(self, obj, user=None) -> Dict[str, Any]:
        ...
        return {
            ...
            if obj.last_login and user and (user.id == obj.id or user.is_staff)
            else None,
    ...
    def register_crud_routes(self) -> None:
        ...
        def list_users(request: HttpRequest, query: UserQuerySchema = Query(...)):
            ...
            try:
                ...
                RequestLogger.info(
                    "管理员获取用户列表", admin_id=user.id, query_params=query.dict()
                )
                ...
                RequestLogger.info(
                    "用户列表查询完成", total_count=total, page=query.page
                )
                ...
                RequestLogger.error(
                    "获取用户列表失败", error=str(e), admin_id=getattr(user, "id", None)
                )
                raise OperationError("获取用户列表失败")

        @self.router.post(
            "/", response=ApiResponseSchema, tags=["用户管理"], summary="创建用户"
        )
        @log_api_call(
            "创建用户", include_request_data=True, exclude_fields=["password"]
        )
        def create_user(request: HttpRequest, user_data: UserCreateSchema):
            ...
            try:
                ...
                RequestLogger.info(
                    "用户创建成功", admin_id=current_user.id, created_user_id=user.id
                )
        ...
        @log_api_call(
            "更新用户信息", include_request_data=True, exclude_fields=["password"]
        )
        def update_user(
            request: HttpRequest, user_data: UserUpdateSchema, user_id: int = Path(...)
        ):
            """更新用户信息"""
            try:
                current_user = get_current_user(request)
                ...
                user = self.service_class.update_user(
        ...
        def delete_user(request: HttpRequest, user_id: int = Path(...)):
            ...
            try:
                current_user = require_staff(request)
                RequestLogger.info(
                    "管理员删除用户", admin_id=current_user.id, target_user_id=user_id
                )
                ...
                RequestLogger.info(
                    "用户删除成功", admin_id=current_user.id, target_user_id=user_id
                )

                return create_success_response(
                    message="删除用户成功", request_id=get_request_id(request)
                )
    ...
    def register_custom_routes(self) -> None:
        ...
        @log_api_call(
            "更新当前用户信息", include_request_data=True, exclude_fields=["password"]
        )
        ...
        def change_password(request: HttpRequest, data: PasswordChangeSchema):
            ...
            try:
                ...
                return create_success_response(
                    message="密码修改成功", request_id=get_request_id(request)
                )

            except (ValidationError, PermissionError) as e:
                RequestLogger.warning(
                    "密码修改失败", user_id=getattr(user, "id", None), error=str(e)
                )
                raise e
            except Exception as e:
                RequestLogger.error(
                    "密码修改异常", user_id=getattr(user, "id", None), error=str(e)
                )
        ...
        def get_user_profile(request: HttpRequest):
            ...
            try:
                ...
                RequestLogger.error(
                    "获取用户配置失败", user_id=getattr(user, "id", None), error=str(e)
                )
        ...
        def update_user_profile(request: HttpRequest, data: UserProfileSchema):
            ...
            try:
                ...
                RequestLogger.warning(
                    "更新用户配置失败", user_id=getattr(user, "id", None), error=str(e)
                )
                raise e
            except Exception as e:
                RequestLogger.error(
                    "更新用户配置异常", user_id=getattr(user, "id", None), error=str(e)
                )
...
__all__ = ["UserController", "user_controller", "router", "users_router"]
